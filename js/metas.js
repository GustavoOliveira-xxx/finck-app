document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function carregar() {
    [metas, perfil, recorrentes, movimentos] = await Promise.all([ S.listar("goals", {
      ordem: "created_at",
      asc: false
    }), S.obterPerfil(), S.listar("recurring_transactions"), S.listar("goal_movements", {
      ordem: "date",
      asc: false
    }) ]);
    renderResumo();
  ...
  function renderAlvo() {
    ...
    const ordenadas = (emAberto.length ? emAberto : abertas).slice().sort((a, b) => U.progresso(b.current_amount, b.target_amount) - U.progresso(a.current_amount, a.target_amount));
    ...
    window.FinckFX?.mirarAlvo?.(progresso, foco ? foco.name : null, foco ? falta > 0 ? `faltam ${U.moeda(falta)}` : "meta concluída" : null);
  ...
  const rendaLivre = () => {
    const renda = Number(perfil?.income_monthly || 0);
    const fixas = recorrentes.filter(r => r.active !== false && r.type === "saida").reduce((s, r) => s + Number(r.amount || 0), 0);
  ...
  document.getElementById("formMeta").addEventListener("submit", async e => {
    ...
    try {
      ...
      if (id) {
        ...
        const nova = await S.inserir("goals", {
          ...semSaldo,
          current_amount: 0
        });
        if (current_amount > 0) {
        ...
        await G.premiar("meta_criada", {
          motivo: "nova meta criada"
        });
      }
  ...
  document.getElementById("formAporte").addEventListener("submit", async e => {
    ...
    if (modoAporte === "retirada" && valor > guardado + .005) {
    ...
    try {
      if (modoAporte === "retirada") {
        ...
        await G.premiar("meta_aporte", {
          motivo: "aporte em meta"
        });
        const alvo = Number(meta?.target_amount || 0);
        if (alvo > 0 && guardado + valor >= alvo) {
          await G.premiar("meta_concluida", {
            chave: String(id),
            motivo: "meta concluída"
          });
        }
  ...
  async function estornarDaMeta(transacaoId, metaId) {
    ...
    try {
      ...
      U.toast(r.repetida ? "Este movimento já estava estornado." : "Movimento estornado. O histórico foi preservado.", r.repetida ? "info" : "sucesso");
