document.addEventListener("DOMContentLoaded", () => {
  ...
  function initCaptcha(sliderId, textoId, btnId) {
    ...
    let max = 0;
    const medir = () => {
      max = track.clientWidth - slider.clientWidth - 4;
    };
    medir();
    ...
    document.addEventListener("touchmove", e => mover(e.touches[0].clientX), {
      passive: true
    });
  ...
  if (formLogin) {
    ...
    formLogin.addEventListener("submit", async e => {
      ...
      if (!email || !senha) {
        return msg(mensagemEl, "Preencha e-mail e senha.", "erro");
      }
  ...
  if (formCadastro) {
    ...
    formCadastro.addEventListener("submit", async e => {
      ...
      if (!nome) {
        return msg(mensagemEl, "Informe seu nome.", "erro");
      }
      if (!/^\S+@\S+\.\S+$/.test(email)) {
        return msg(mensagemEl, "Informe um e-mail válido.", "erro");
      }
      if (senha.length < 8) {
        return msg(mensagemEl, "A senha precisa ter ao menos 8 caracteres.", "erro");
      }
      if (senha !== confirma) {
        return msg(mensagemEl, "As senhas não conferem.", "erro");
      }
      if (!aceite) {
        return msg(mensagemEl, "É necessário aceitar o uso educacional.", "erro");
      }
      const btn = document.getElementById("btnCadastrar");
      btn.disabled = true;
      try {
        const {precisaConfirmar: precisaConfirmar} = await S.cadastrar({
          nome: nome,
          email: email,
          senha: senha
        });
  ...
  if (formRecuperar) {
    ...
    formRecuperar.addEventListener("submit", async e => {
      ...
      try {
        ...
        msg(mensagemEl, "Se existir uma conta com esse e-mail, o link de redefinição já está a caminho. O link vale por 1 hora.", "sucesso");
        formRecuperar.reset();
      } catch (err) {
        msg(mensagemEl, err.message, "erro");
      } finally {
