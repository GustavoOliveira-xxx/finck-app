window.FinckGame = (() => {
  ...
  const CONQUISTAS = [ {
    ...
    descricao: "Configurou o perfil financeiro.",
    teste: c => Boolean(c.perfilCompleto)
  }, {
    id: "primeiro_lancamento",
    icone: "✍️",
    titulo: "Mão na massa",
    descricao: "Registrou a primeira movimentação.",
    teste: c => c.transacoes >= 1
  }, {
    id: "constancia",
    icone: "📒",
    titulo: "Constância",
    descricao: "Registrou 10 movimentações.",
    teste: c => c.transacoes >= 10
  }, {
    id: "diario_de_bordo",
    icone: "📚",
    titulo: "Diário de bordo",
    descricao: "Registrou 50 movimentações.",
    teste: c => c.transacoes >= 50
  }, {
    id: "primeira_meta",
    icone: "🎯",
    titulo: "Com um plano",
    descricao: "Criou a primeira meta.",
    teste: c => c.metas >= 1
  }, {
    id: "meta_concluida",
    icone: "🏁",
    ...
    teste: c => c.metasConcluidas >= 1
  }, {
    id: "colecionador_metas",
    icone: "🏆",
    titulo: "Colecionador de metas",
    descricao: "Concluiu 3 metas financeiras.",
    teste: c => c.metasConcluidas >= 3
  }, {
    id: "primeira_analise",
    icone: "🧮",
    titulo: "Pensou antes",
    ...
    descricao: "Adiou, substituiu ou desistiu de 5 compras.",
    teste: c => c.evitadas >= 5
  }, {
    id: "freio_de_mao",
    icone: "🛑",
    titulo: "Freio de mão",
    descricao: "Evitou 20 compras por impulso.",
    teste: c => c.evitadas >= 20
  }, {
    id: "guardiao",
    icone: "⏳",
    titulo: "Guardião do tempo",
    descricao: "Preservou 100 horas de trabalho evitando compras.",
    teste: c => c.horasPreservadas >= 100
  }, {
    id: "ano_de_vida",
    icone: "🕰️",
    ...
    id: "primeira_entrada",
    icone: "💰",
    titulo: "Primeiro dinheiro registrado",
    descricao: "Cadastrou a primeira entrada.",
    teste: c => c.entradas >= 1
  }, {
    id: "primeira_saida",
    icone: "💸",
    titulo: "Primeiro gasto registrado",
    descricao: "Cadastrou a primeira saída.",
    teste: c => c.saidas >= 1
  }, {
    id: "mestre_financas",
    icone: "🏅",
    ...
    icone: "🔥",
    titulo: "Semana firme",
    descricao: "Manteve 7 dias seguidos de uso do app.",
    teste: c => c.streak >= 7
  }, {
    id: "mes_firme",
    icone: "☄️",
    titulo: "Mês firme",
    descricao: "Manteve 30 dias seguidos de uso do app.",
    teste: c => c.streak >= 30
  } ];
  ...
  function normalizarLedger(estado) {
    ...
    return {
      ...
      acoes: l.acoes || {},
      ultimo: Number(l.ultimo || 0),
      chaves: chaves
    };
  }
  function podePremiar(estado, tipo, chave, {ignorarIntervalo: ignorarIntervalo = false} = {}) {
    ...
    if (chave && l.chaves.includes(`${tipo}:${chave}`)) {
      return {
        ok: false,
        motivo: "Esta ação já rendeu XP antes."
      };
    }
    if (l.total >= REGRAS.TETO_DIARIO) {
      return {
        ok: false,
        motivo: "Teto diário de XP atingido. Volte amanhã."
      };
    }
    if ((l.acoes[tipo] || 0) >= regra.limiteDia) {
      return {
        ok: false,
        motivo: `Limite diário de "${regra.rotulo}" atingido.`
      };
    }
    if (!ignorarIntervalo && Date.now() - l.ultimo < REGRAS.INTERVALO_MIN_MS) {
      return {
        ok: false,
        motivo: "Aguarde alguns segundos entre ações."
      };
    }
    return {
      ok: true,
  ...
  async function premiar(tipo, opcoes = {}) {
    ...
    if (estado.last_active === hoje) {} else if (estado.last_active === ontem) {
      streak += 1;
    } else {
      streak = 1;
    }
    ...
    const ganho = Math.max(0, Math.min(Number(opcoes.xp ?? veredito.regra.xp), REGRAS.TETO_DIARIO - ledger.total));
    ledger.total += ganho;
  ...
  return {
    ...
    premiar: premiar,
    ganharXP: ganharXP,
    podePremiar: podePremiar,
    sincronizarConquistas: sincronizarConquistas,
