window.FinckCenarios = (() => {
  ...
  function fluxograma(entrada, cenarios) {
    ...
    cenarios.forEach((c, i) => {
      ...
      const noResultado = {
        id: `resultado-${c.id}`,
        tipo: "resultado",
        titulo: c.custoMensal && c.parcelas ? `${c.parcelas}x de ${moeda(c.custoMensal)}` : c.precoEfetivo > 0 ? moeda(c.precoEfetivo) : "Nada sai do bolso",
        linhas: [ {
          rotulo: "Saldo depois",
          valor: moeda(c.saldoDepois)
        }, {
          rotulo: "Em trabalho",
          valor: c.horas > 0 ? formatarHoras(c.horas) : "—"
        }, c.impactoMetas.possui ? {
          rotulo: "Metas",
          valor: `${Math.round(c.impactoMetas.percentual)}%`
        } : null ].filter(Boolean),
        risco: risco,
        nivel: 2,
        ordem: i
  ...
  function formatarHoras(h) {
    if (window.FinckTempo) {
      return window.FinckTempo.formatar(h);
    }
