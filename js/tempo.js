window.FinckTempo = (() => {
  ...
  function formatar(horas, {longo: longo = false} = {}) {
  ...
  function selo(valor, {classe: classe = "", titulo: titulo = true} = {}) {
    if (!ativo()) {
      return "";
    }
    ...
    const completo = `${formatar(horasDe(v), {
      longo: true
    })} pelo seu valor-hora atual`;
    return `<span class="selo-tempo ${classe}"${titulo ? ` title="${completo}"` : ""}>` + `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" aria-hidden="true">` + `<circle cx="12" cy="12" r="9"/><path d="M12 7.5V12l3 2"/></svg>` + `<span>${texto}</span></span>`;
  }
  const seloTexto = valor => ativo() ? formatar(horasDe(valor), {
    longo: true
  }) : "";
  return {
    definirPerfil: definirPerfil,
    valorHora: valorHora,
    ativo: ativo,
    horasDe: horasDe,
    formatar: formatar,
