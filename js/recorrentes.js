document.addEventListener("DOMContentLoaded", async () => {
  ...
  let contas = [];
  let editando = null;
  document.getElementById("recCategoria").innerHTML = cfg.CATEGORIAS.map(c => `<option value="${c}">${c}</option>`).join("");
  async function carregar() {
    [itens, contas] = await Promise.all([ S.listar("recurring_transactions", {
      ordem: "day_of_month",
      asc: true
    }), S.listar("accounts") ]);
  ...
  function render() {
    ...
    host.innerHTML = itens.map(i => {
      ...
      const detalhes = [ `dia ${i.day_of_month}`, i.type === "saida" ? i.category || "sem categoria" : null, conta || "sem conta" ].filter(Boolean).join(" · ");
      return `\n      <li class="item-lista ${i.active === false ? "inativo" : ""}">\n        <span class="item-desc">${U.escapeHTML(i.description)}</span>\n        <span class="item-dia">${U.escapeHTML(detalhes)}</span>\n        <strong class="${i.type === "entrada" ? "cor-verde" : "cor-vermelha"}">\n          ${i.type === "entrada" ? "+" : "−"} ${U.moeda(i.amount)}\n        </strong>\n        ${T.selo(i.amount, {
        classe: i.type === "entrada" ? "selo-tempo--entrada" : ""
    ...
    host.querySelectorAll("[data-ativo]").forEach(c => c.addEventListener("change", async () => {
      await S.atualizar("recurring_transactions", c.dataset.ativo, {
        active: c.checked
      });
      carregar();
    }));
  ...
  async function limparPrevisoes(recurringId) {
    ...
    for (const oc of abertas) {
      await S.remover(OCORRENCIAS, oc.id);
    }
  }
  async function ocorrenciasAbertas(recurringId) {
    const todas = await S.listar(OCORRENCIAS, {
      filtro: {
  ...
  function perguntarPropagacao(quantas, resumo) {
    return new Promise(resolver => {
      $("textoPropagar").textContent = `${quantas} ${quantas === 1 ? "mês ainda não foi revisado" : "meses ainda não foram revisados"} e ${quantas === 1 ? "guarda" : "guardam"} o valor antigo (${resumo}).`;
      ...
      modal.querySelectorAll("[data-propagar]").forEach(b => b.addEventListener("click", () => escolher(b.dataset.propagar), {
        once: true
      }));
  ...
  $("formRecorrente").addEventListener("submit", async e => {
    ...
    try {
      if (editando) {
        ...
        if (abertas.length) {
          const escolha = await perguntarPropagacao(abertas.length, `previsto de ${U.moeda(editando.amount)}`);
          if (escolha === "abertas") {
            await propagarParaAbertas(regra, abertas);
          }
        }
