window.FinckFechamento = (() => {
  ...
  const soma = (lista, campo) => (lista || []).reduce((s, r) => s + (Number(r[campo]) || 0), 0);
  const doCiclo = (lista, ciclo, campoData = "date") => (lista || []).filter(r => String(r[campoData] || "").slice(0, 7) === ciclo);
  function referencia(perfil, chave, receita) {
    ...
    if (modo === "valor" && temFixo) {
      return {
        valor: fixo,
        base: "valor"
      };
    }
    if (modo === "percentual" && temPct) {
      return {
        valor: receita * pct / 100,
        base: "percentual"
      };
    }
    if (temPct) {
  ...
  function resumo(ciclo, {ocorrencias: ocorrencias, transacoes: transacoes, perfil: perfil, categoriasLivres: categoriasLivres}) {
    ...
    const livres = categoriasLivres && categoriasLivres.length ? categoriasLivres : [ "Lazer", "Vestuário", "Eletrônicos" ];
    const rendaLivreUsada = soma(movs.filter(m => m.type === "saida" && livres.includes(m.category)), "amount");
    const economiaRealizada = soma(movs.filter(m => m.type === "saida" && m.goal_id), "amount");
    ...
    return {
      ...
      despesaEsperada: despesaEsperada,
      despesaRealizada: despesaRealizada,
      recorrentesRealizados: recorrentesRealizados,
      ...
      rendaLivreUsada: rendaLivreUsada,
      ...
      economiaRealizada: economiaRealizada,
