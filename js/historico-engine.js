window.FinckHistorico = (() => {
  ...
  const rotuloMes = chave => {
    ...
    return new Date(ano, mes - 1, 1).toLocaleDateString(cfg.LOCALE, {
      month: "short",
      year: "2-digit"
    });
  };
  ...
  const saidasDoMes = (transacoes, mes, categoria = null) => transacoes.filter(t => t.type === "saida" && String(t.date || "").slice(0, 7) === mes).filter(t => categoria ? (t.category || "Outros") === categoria : true).reduce((s, t) => s + num(t.amount), 0);
  function serieDaCategoria(transacoes, categoria, mesRef, meses = 6) {
    ...
    const mediaAnterior = anteriores.length ? anteriores.reduce((s, l) => s + l.total, 0) / anteriores.length : 0;
  ...
  function panorama(transacoes, mesRef, meses = 6) {
    const categorias = [ ...new Set(transacoes.filter(t => t.type === "saida" && String(t.date || "").slice(0, 7) === mesRef).map(t => t.category || "Outros")) ];
  ...
  function observacao(linha) {
    ...
    const moeda = v => num(v).toLocaleString(cfg.LOCALE, {
      style: "currency",
      currency: cfg.MOEDA
    });
  ...
  const tendencia = linha => {
    if (!linha.temHistorico || Math.abs(linha.variacao) < 10) {
      return "estavel";
    }
  ...
  return {
    mesesAte: mesesAte,
    rotuloMes: rotuloMes,
    serieDaCategoria: serieDaCategoria,
    panorama: panorama,
