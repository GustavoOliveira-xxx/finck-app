window.FinckContas = (() => {
  ...
  function instituicao(id) {
    return cfg.INSTITUICOES.find(i => i.id === id) || cfg.INSTITUICOES.find(i => i.id === "outro");
  }
  const rotuloTipo = id => (cfg.TIPOS_CONTA.find(t => t.id === id) || {
    rotulo: "Conta"
  }).rotulo;
  function saldoDaConta(conta, {transacoes: transacoes = [], transferencias: transferencias = [], ajustes: ajustes = []} = {}) {
    const id = String(conta.id);
    let saldo = num(conta.initial_balance);
    transacoes.forEach(t => {
      if (String(t.account_id) !== id) {
        return;
      }
      saldo += t.type === "entrada" ? num(t.amount) : -num(t.amount);
    });
    transferencias.forEach(t => {
      if (String(t.from_account_id) === id) {
        saldo -= num(t.amount);
      }
      if (String(t.to_account_id) === id) {
        saldo += num(t.amount);
      }
    });
    ajustes.forEach(a => {
      if (String(a.account_id) === id) {
        saldo += num(a.amount);
      }
    });
    return saldo;
  ...
  function validarTransferencia({origem: origem, destino: destino, valor: valor}) {
    if (!origem || !destino) {
      return "Escolha a conta de origem e a de destino.";
    }
    if (String(origem) === String(destino)) {
      return "A conta de destino precisa ser diferente da origem.";
    }
    if (!(num(valor) > 0)) {
  ...
  return {
    ...
    porInstituicao: porInstituicao,
    validarTransferencia: validarTransferencia,
    diferencaDoAjuste: diferencaDoAjuste,
    semConta: semConta
  };
})();
