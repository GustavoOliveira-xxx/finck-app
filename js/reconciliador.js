window.FinckReconciliador = (() => {
  ...
  function conferir({perfil: perfil = null, contas: contas = [], transacoes: transacoes = [], transferencias: transferencias = [], ajustes: ajustes = [], metas: metas = [], movimentosMeta: movimentosMeta = [], parcelamentos: parcelamentos = [], pagamentos: pagamentos = [], ocorrencias: ocorrencias = [], hoje: hoje = window.FinckUtils.hojeISO()} = {}) {
    ...
    const parcelasAbertas = (parcelamentos || []).filter(p => p.active !== false).flatMap(p => P.cronograma(p, porCompra.get(String(p.id)) || []).filter(c => !c.paga));
    ...
    const compromissoOcorrencias = ocorrenciasAbertas.filter(o => o.type === "saida").reduce((s, o) => s + num(o.planned_amount), 0);
    const idsVivos = new Set(noCaixa.map(t => String(t.id)));
    const previsoesComDinheiro = ocorrenciasAbertas.filter(o => o.transaction_id && idsVivos.has(String(o.transaction_id)));
    const parcelasAbertasComMovimento = (pagamentos || []).filter(pg => pg.status === "aberta" && pg.transaction_id);
    const grupos = new Map;
    noCaixa.forEach(t => {
      const chave = [ t.type, Number(t.amount).toFixed(2), String(t.date).slice(0, 10), String(t.description || "").trim().toLowerCase() ].join("|");
      if (!grupos.has(chave)) {
        grupos.set(chave, []);
      }
      grupos.get(chave).push(t);
    });
    const duplicatas = [ ...grupos.values() ].filter(lista => lista.length > 1).filter(lista => {
    ...
    const checagens = [ {
      ...
      formato: "moeda",
      explicacao: bate(saldoGlobal, identidade) ? somaArquivadas ? `O saldo que aparece na tela é a soma das contas ativas, mais ${window.FinckUtils.moeda(somaArquivadas)} que passou por contas arquivadas, mais o que está fora das contas.` : "O saldo que aparece na tela é exatamente a soma das contas mais o que está fora delas." : "O saldo global não corresponde à soma das contas com o não alocado. Enquanto isso não fechar, os dois números na tela contam histórias diferentes."
    }, {
      id: "contas",
      ...
      explicacao: comSaldo.some(c => c.saldo < 0) ? `${comSaldo.filter(c => c.saldo < 0).length} conta(s) com saldo negativo. Pode ser cheque especial legítimo — só precisa ser uma escolha, não uma surpresa.` : "Cada conta soma saldo inicial, movimentações vinculadas, transferências e ajustes."
    }, {
      id: "nao_alocado",
      ...
      explicacao: ambiguas.length ? `${ambiguas.length} movimentação(ões) sem conta e sem você ter dito que ficam fora das contas. Elas entram no saldo geral mas não aparecem em conta nenhuma.` : "Tudo que está fora das contas está fora por decisão declarada."
    }, {
      id: "categorias",
      ...
      alerta: semCategoria,
      explicacao: semCategoria ? `${semCategoria} saída(s) sem categoria caem em "Outros" nos relatórios.` : "O total por categoria reproduz o total de saídas."
    }, {
      id: "metas",
      ...
      explicacao: divergentes.length ? `${divergentes.length} meta(s) com progresso diferente da soma dos movimentos: ${divergentes.map(d => d.nome).join(", ")}.` : metasSemHistorico.length ? `${metasSemHistorico.length} meta(s) ainda sem histórico de movimentos — o valor veio de antes do livro-razão.` : "O valor guardado em cada meta é exatamente a soma dos aportes, retiradas e estornos."
    }, {
      id: "compromissos",
      ...
      explicacao: previsoesComDinheiro.length || parcelasAbertasComMovimento.length ? "Existe previsão em aberto com movimentação vinculada. O mesmo dinheiro está sendo contado como previsto e como realizado." : "Nada em aberto está movimentando saldo. Previsto é previsto; realizado é realizado."
    } ];
    const pendencias = [];
    ...
    (parcelamentos || []).filter(p => num(p.paid_count) > 0 && !(porCompra.get(String(p.id)) || []).some(pg => pg.status === "paga")).forEach(p => pendencias.push({
    ...
    (ocorrencias || []).filter(o => o.type === "saida" && !o.category).slice(0, 25).forEach(o => pendencias.push({
    ...
    if (perfil && num(perfil.initial_balance) !== 0 && !perfil.initial_balance_migrated_at && ativas.length) {
      pendencias.push({
    ...
    return {
      ...
      caixa: {
        ...
        somaContas: somaContas,
        somaArquivadas: somaArquivadas,
        contasArquivadas: arquivadas.length,
        naoAlocado: naoAlocado,
        naoAlocadoMovimento: naoAlocadoMovimento,
      ...
      metas: {
        ...
        semHistorico: metasSemHistorico
      },
      compromissos: {
  ...
  async function carregar() {
    const [perfil, contas, transacoes, transferencias, ajustes, metas, movimentosMeta, parcelamentos, pagamentos, ocorrencias] = await Promise.all([ S.obterPerfil(), S.listar("accounts"), S.listar("transactions", {
      ordem: "date",
      asc: false
    }), S.listar("transfers"), S.listar("balance_adjustments"), S.listar("goals"), S.listar("goal_movements", {
      ordem: "date",
      asc: false
    }), S.listar("installment_purchases"), S.listar("installment_payments"), S.listar("recurring_occurrences", {
      ordem: "due_date",
      asc: true
    }) ]);
    return {
  ...
  async function registrarPendencias(pendencias = []) {
    ...
    const abertas = new Set(existentes.filter(e => e.status !== "resolvido").map(chave));
    let novas = 0;
    for (const p of pendencias) {
      if (abertas.has(chave(p))) {
        continue;
      }
