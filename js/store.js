window.FinckStore = (() => {
  ...
  async function usuarioAtual() {
    ...
    if (sb) {
      const {data: data} = await sb.auth.getUser();
      return data?.user ? {
        id: data.user.id,
  ...
  const SEM_BANCO = "Cadastro e login precisam do banco de dados configurado. " + "Enquanto isso, use o modo demonstração para conhecer o app.";
  const urlLocal = (arquivo, base = location.href) => new URL(arquivo, base).href;
  async function cadastrar({nome: nome, email: email, senha: senha}) {
    if (!sb) {
      throw new Error(SEM_BANCO);
    }
    const {data: data, error: error} = await sb.auth.signUp({
      ...
      options: {
        ...
        emailRedirectTo: urlLocal("index.html")
      }
    });
    if (error) {
  ...
  async function recuperarSenha(email) {
    ...
    const {error: error} = await sb.auth.resetPasswordForEmail(email, {
  ...
  async function reenviarConfirmacao(email) {
    ...
    const {error: error} = await sb.auth.resend({
      type: "signup",
      email: email,
      options: {
        emailRedirectTo: urlLocal("index.html")
  ...
  async function listar(tabela, {ordem: ordem = "created_at", asc: asc = false, filtro: filtro = {}} = {}) {
    const user = await usuarioAtual();
    if (!user) {
      return [];
    }
    ...
    if (banco) {
      ...
      const {data: data, error: error} = await q.order(ordem, {
        ascending: asc
      });
      if (error) {
  ...
  async function inserir(tabela, registro) {
    ...
    const banco = bd();
    if (banco) {
      const {data: data, error: error} = await banco.from(tabela).insert(linha).select().single();
      if (error) {
        throw new Error(error.message);
      }
      return data;
  ...
  async function upsert(tabela, registros, chaves) {
    ...
    const banco = bd();
    if (banco) {
      const {data: data, error: error} = await banco.from(tabela).upsert(linhas, {
        onConflict: chaves.join(","),
    ...
    const existe = r => atuais.some(a => a.user_id === user.id && chaves.every(k => String(a[k]) === String(r[k])));
    ...
    if (novas.length) {
      gravar(KEYS[tabela], [ ...novas, ...atuais ]);
    }
    return novas;
  }
  ...
  async function atualizar(tabela, id, campos) {
    ...
    const banco = bd();
    if (banco) {
      const {data: data, error: error} = await banco.from(tabela).update(campos).eq("id", id).eq("user_id", user.id).select().single();
      if (error) {
        throw new Error(error.message);
      }
      return data;
  ...
  async function remover(tabela, id) {
    ...
    const banco = bd();
    if (banco) {
      const {error: error} = await banco.from(tabela).delete().eq("id", id).eq("user_id", user.id);
      if (error) {
        throw new Error(error.message);
      }
      return true;
    ...
    if (restantes.length === linhas.length) {
      return false;
    }
  ...
  async function rpc(nome, parametros = {}) {
    ...
    const {data: data, error: error} = await banco.rpc(nome, parametros);
    if (error) {
  ...
  async function operacao(chave, executar, {operacao: nome = "operacao"} = {}) {
    ...
    if (emVoo.has(registro)) {
      return emVoo.get(registro);
    }
    const corpo = async () => {
      const registradas = ler(KEYS.operation_keys, []);
      const guardada = registradas.find(r => r.user_id === user.id && r.key === chave);
      if (guardada) {
        return guardada.result;
      }
  ...
  const chaveDeOperacao = (...partes) => `${partes.filter(p => p !== null && p !== undefined).join(":")}`;
  const LIMITE_EVENTOS = 200;
  async function registrarEvento({level: level = "erro", scope: scope, message: message, context: context = {}}) {
    const linha = {
    ...
    try {
      ...
      const banco = bd();
      if (banco) {
        const {id: id, ...semId} = linha;
        await banco.from("integrity_events").insert(semId);
      }
    } catch {}
    return linha;
  }
  async function listarEventos({limite: limite = 50} = {}) {
    const user = await usuarioAtual();
    if (!user) {
      return [];
    }
    const locais = ler(KEYS.integrity_events, []).filter(e => !e.user_id || e.user_id === user.id);
    const banco = bd();
    if (!banco) {
      return locais.slice(0, limite);
    }
    try {
      const {data: data} = await banco.from("integrity_events").select("*").eq("user_id", user.id).order("created_at", {
        ascending: false
      }).limit(limite);
      const vistos = new Set((data || []).map(e => `${e.created_at}|${e.message}`));
      const extras = locais.filter(e => !vistos.has(`${e.created_at}|${e.message}`));
      return [ ...data || [], ...extras ].sort((a, b) => String(b.created_at).localeCompare(String(a.created_at))).slice(0, limite);
    } catch {
      return locais.slice(0, limite);
    }
  }
  async function limparEventos() {
    ...
    if (banco && user) {
      try {
        await banco.from("integrity_events").delete().eq("user_id", user.id);
      } catch {}
    }
    return true;
  }
  async function obter(tabela, id) {
    ...
    const banco = bd();
    if (banco) {
      const {data: data, error: error} = await banco.from(tabela).select("*").eq("id", id).eq("user_id", user.id).maybeSingle();
      if (error) {
        throw new Error(error.message);
      }
      return data;
  ...
  async function obterPerfil() {
    ...
    const banco = bd();
    if (banco) {
      const {data: data, error: error} = await banco.from("profiles").select("*").eq("id", user.id).maybeSingle();
      if (error) {
        throw new Error(error.message);
      }
      return data;
  ...
  async function salvarPerfil(campos) {
    ...
    const banco = bd();
    if (banco) {
      const {data: data, error: error} = await banco.from("profiles").upsert(registro).select().single();
      if (error) {
        throw new Error(error.message);
      }
      return data;
  ...
  async function precisaOnboarding() {
    ...
    return !p || !p.onboarded_at || !(Number(p.work_days_month) >= 1 && Number(p.work_days_month) <= 31) || !(Number(p.work_hours_day) > 0 && Number(p.work_hours_day) <= 16);
  }
  async function obterGamificacao() {
    ...
    const banco = bd();
    if (banco) {
      const {data: data} = await banco.from("gamification").select("*").eq("user_id", user.id).maybeSingle();
      return data || base;
    }
  ...
  async function salvarGamificacao(estado) {
    ...
    const banco = bd();
    if (banco) {
      const {data: data, error: error} = await banco.from("gamification").upsert(registro, {
        onConflict: "user_id"
      }).select().single();
      if (error) {
        throw new Error(error.message);
  ...
  const VINCULOS = {
    ...
    installment_payments: [ [ "purchase_id", "installment_purchases", true ], [ "transaction_id", "transactions", false ] ],
    goal_movements: [ [ "goal_id", "goals", true ], [ "transaction_id", "transactions", false ], [ "reverses_id", "goal_movements", false ] ]
  };
  ...
  const limparCampos = (tabela, linha, {manterId: manterId = false} = {}) => {
    const permitido = new Set(CAMPOS_PERMITIDOS[tabela] || []);
    const limpa = {};
    if (manterId && linha?.id !== undefined && linha?.id !== null) {
      limpa.id = linha.id;
    }
    for (const campo of permitido) {
      if (Object.prototype.hasOwnProperty.call(linha || {}, campo)) {
        limpa[campo] = linha[campo];
      }
    }
    return limpa;
  };
  ...
  const INVARIANTES = {
    accounts: r => !String(r.name || "").trim() ? "conta sem nome" : r.initial_balance_date && !dataValida(r.initial_balance_date) ? "conta com data inicial inválida" : r.initial_balance !== undefined && !Number.isFinite(Number(r.initial_balance)) ? "conta com saldo inicial inválido" : null,
    goals: r => !String(r.name || "").trim() ? "meta sem nome" : !positivo(r.target_amount) ? "meta com alvo zerado ou negativo" : !naoNegativo(r.current_amount) ? "meta com valor guardado negativo" : r.deadline && !dataValida(r.deadline) ? "meta com prazo inválido" : null,
    transactions: r => ![ "entrada", "saida" ].includes(r.type) ? "movimentação sem tipo válido" : !positivo(r.amount) ? "movimentação com valor zerado ou negativo" : !dataValida(r.date) ? "movimentação sem data válida" : !String(r.description || "").trim() ? "movimentação sem descrição" : null,
    recurring_transactions: r => ![ "entrada", "saida" ].includes(r.type) ? "recorrente sem tipo válido" : !positivo(r.amount) ? "recorrente com valor zerado ou negativo" : !String(r.description || "").trim() ? "recorrente sem descrição" : !(Number(r.day_of_month) >= 1 && Number(r.day_of_month) <= 31) ? "recorrente com dia fora de 1 a 31" : null,
    installment_purchases: r => !String(r.description || "").trim() ? "parcelamento sem descrição" : !positivo(r.total_amount) ? "parcelamento com total zerado ou negativo" : !(Number.isInteger(Number(r.installments_count)) && Number(r.installments_count) >= 1 && Number(r.installments_count) <= 120) ? "parcelamento sem número de parcelas válido" : !positivo(r.installment_amount) ? "parcelamento com valor de parcela inválido" : !dataValida(r.first_due_date) ? "parcelamento sem primeiro vencimento válido" : !(Number.isInteger(Number(r.paid_count || 0)) && Number(r.paid_count || 0) >= 0) ? "parcelamento com quantidade paga inválida" : Number(r.paid_count || 0) > Number(r.installments_count) ? "parcelamento com mais parcelas pagas que o total" : null,
  ...
  function validarImportacao(dados) {
    ...
    if (!dados || typeof dados !== "object" || Array.isArray(dados)) {
      return {
        valido: false,
        erros: [ "Arquivo inválido: não parece um backup do FinCK." ],
        avisos: avisos,
        linhas: {}
      };
    }
    ...
    if (!temAlgo) {
      return {
        valido: false,
        erros: [ "Arquivo inválido: nenhum dado reconhecido." ],
        avisos: avisos,
        linhas: {}
      };
    }
    const idsNoArquivo = {};
    TABELAS.forEach(t => {
      idsNoArquivo[t] = new Set((Array.isArray(dados[t]) ? dados[t] : []).map(r => r && r.id !== undefined && r.id !== null ? String(r.id) : null).filter(Boolean));
    });
    ...
    for (const tabela of TABELAS) {
      ...
      (bruto || []).forEach((linha, i) => {
        ...
        if (motivo) {
          avisos.push(`${posicao}: ${motivo} — registro descartado.`);
          return;
        }
        let descartar = false;
  ...
  async function inserirSeNovo(tabela, registro, existentes = null) {
    ...
    if (atuais.some(r => assinar(tabela, r) === alvo)) {
      return null;
    }
  ...
  async function importarTudo(dados, {modo: modoImport = "mesclar"} = {}) {
    ...
    for (const t of TABELAS) {
      ...
      const linhas = t === "goal_movements" ? [ ...linhasBase ].sort((a, b) => Number(Boolean(a.reverses_id)) - Number(Boolean(b.reverses_id))) : linhasBase;
      ...
      for (const linha of linhas) {
        const {id: id, user_id: user_id, created_at: created_at, ...resto} = linha;
        ...
        if (porAssinatura.has(chave)) {
          ...
          if (relatorio.conflitos.length < 25) {
            relatorio.conflitos.push({
              tabela: t,
              descricao: descreverLinha(t, preparada)
            });
          }
          continue;
        }
  ...
  const descreverLinha = (tabela, r) => {
    if (tabela === "transactions") {
      return `${r.description} · ${r.date}`;
    }
    if (tabela === "goals") {
      return r.name;
    }
    if (tabela === "recurring_transactions") {
      return `${r.description} · dia ${r.day_of_month}`;
    }
    if (tabela === "purchase_analyses") {
      return r.item_name;
    }
  ...
  return {
    ...
    obter: obter,
    inserir: inserir,
    inserirSeNovo: inserirSeNovo,
    upsert: upsert,
    atualizar: atualizar,
    remover: remover,
    rpc: rpc,
    operacao: operacao,
    chaveDeOperacao: chaveDeOperacao,
    registrarEvento: registrarEvento,
    listarEventos: listarEventos,
    limparEventos: limparEventos,
    obterPerfil: obterPerfil,
    salvarPerfil: salvarPerfil,
    precisaOnboarding: precisaOnboarding,
    obterGamificacao: obterGamificacao,
    salvarGamificacao: salvarGamificacao,
    exportarTudo: exportarTudo,
    importarTudo: importarTudo,
    validarImportacao: validarImportacao,
