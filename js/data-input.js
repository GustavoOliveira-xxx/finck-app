window.FinckData = (() => {
  ...
  function ligar(el) {
    ...
    if (!el.placeholder || /^\s*$/.test(el.placeholder)) {
  ...
  return {
    ligar: ligar,
    ligarTodos: (raiz = document) => raiz.querySelectorAll('input[type="date"], input[data-data]').forEach(ligar),
    paraISO: paraISO,
    deISO: deISO,
    ler: alvo => {
      const el = elemento(alvo);
      return el ? paraISO(el.value) : "";
    },
    escrever: (alvo, iso) => {
      const el = elemento(alvo);
      if (!el) {
