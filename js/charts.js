window.FinckCharts = (() => {
  ...
  const corTexto = () => getComputedStyle(document.documentElement).getPropertyValue("--finck-chart-texto").trim() || "#e5e7eb";
  ...
  function rosca(canvas, dados) {
    ...
    const {ctx: ctx, largura: largura, altura: altura} = preparar(canvas);
    ...
    ctx.fillText(total.toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL"
    }), cx, cy + 5);
  }
  ...
  function tabelaEquivalente(canvas, spec) {
    ...
    const corpo = spec.linhas.map(l => `<tr>${l.map((v, i) => i === 0 ? `<th scope="row">${U().escapeHTML(String(v))}</th>` : `<td>${U().escapeHTML(String(v))}</td>`).join("")}</tr>`).join("");
    ...
    if (spec.resumo) {
      canvas.setAttribute("aria-label", spec.resumo);
    }
  ...
  return {
    ...
    progresso: progresso,
    cores: cores,
    tabelaEquivalente: tabelaEquivalente
  };
})();
