window.FinckProgramacao = (() => {
  ...
  function compromissos({ocorrencias: ocorrencias, recorrentes: recorrentes} = {}, {dias: dias = 60, referencia: referencia = new Date} = {}) {
    ...
    const daOcorrencia = (ocorrencias || []).filter(o => o.type === "saida" && abertos.includes(o.status)).map(o => montarItem(`oc:${o.id || `${o.recurring_id}|${o.cycle}`}`, o.description, o.planned_amount, String(o.due_date), referencia));
    ...
    for (const ciclo of ciclosNaJanela(referencia, limite)) {
      ...
      for (const r of ativos) {
        ...
        projetados.push(montarItem(`re:${r.id}|${ciclo}`, r.description, r.amount, isoData, referencia));
      }
    }
    return [ ...daOcorrencia, ...projetados ].filter(c => c.valor > 0 && c.iso <= isoLimite).sort((a, b) => a.iso.localeCompare(b.iso));
  }
