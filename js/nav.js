window.FinckNav = (() => {
  ...
  const MODOS = {
    online: {
      classe: "online",
      rotulo: "Online",
      ajuda: "Conectado ao banco de dados",
    ...
    demo: {
      classe: "demo",
      rotulo: "Demonstração",
      ajuda: "Dados de exemplo, salvos só neste aparelho",
    ...
    local: {
      classe: "local",
      rotulo: "Sem banco",
      ajuda: "Banco de dados não configurado",
  ...
  async function iniciarPagina({titulo: titulo, subtitulo: subtitulo, exigirPerfil: exigirPerfil = true} = {}) {
    ...
    if (T) {
      T.definirPerfil(await S.obterPerfil());
    }
    const G = window.FinckGame;
    if (G) {
      const hoje = U.hojeISO();
      await G.premiar("primeiro_acesso", {
        chave: hoje,
        motivo: "primeiro acesso do dia",
        silencioso: true
      });
      const estado = await S.obterGamificacao();
      if (Number(estado.streak || 0) >= 2) {
        await G.premiar("streak", {
          chave: hoje,
          motivo: "sequência diária mantida",
          silencioso: true
        });
      }
    }
