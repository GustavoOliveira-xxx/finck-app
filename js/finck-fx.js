(() => {
  ...
  function ligarPonteiro(alvo, aoMover, aoSair) {
    const mover = e => {
      ...
      if (p && typeof p.clientX === "number") {
        aoMover(p.clientX, p.clientY);
      }
    };
    alvo.addEventListener("pointermove", mover, {
      passive: true
    });
    alvo.addEventListener("touchmove", mover, {
      passive: true
    });
    if (aoSair) {
      [ "pointerleave", "pointercancel", "touchend", "touchcancel" ].forEach(ev => alvo.addEventListener(ev, aoSair, {
        passive: true
      }));
    }
  ...
  function abertura() {
    ...
    const pintar = p => {
      if (arco) {
        arco.style.strokeDashoffset = (VOLTA * (1 - p)).toFixed(1);
      }
      if (rotulo) {
        rotulo.textContent = `${Math.round(p * 100)}%`;
      }
      tela.style.setProperty("--carga", p.toFixed(3));
    };
    const correr = () => {
      const decorrido = performance.now() - nasceu;
      const alvo = document.readyState === "complete" ? 1 : .86;
      progresso += (alvo - progresso) * .06;
      if (decorrido > MIN_EM_TELA && alvo === 1) {
        progresso += .02;
      }
    ...
    if (document.readyState === "complete") {
      ...
      window.addEventListener("load", agendarSaida, {
        once: true
      });
    }
  ...
  function prisma() {
    ...
    if (!toque) {
      cena.addEventListener("pointermove", e => {
        ...
        py = ((e.clientY - r.top) / r.height - .5) * -1;
        agendar();
      }, {
        passive: true
      });
  ...
  function medalha() {
    ...
    if (!toque) {
      cena.addEventListener("pointermove", e => {
        ...
        gx = (e.clientX - r.left) / r.width - .5;
        gy = ((e.clientY - r.top) / r.height - .5) * -1;
        luzDoPonteiro(e);
        agendar();
      }, {
        passive: true
  ...
  function montarCristal(cena) {
    if (cena.dataset.montado) {
      return;
    }
    cena.dataset.montado = "1";
    const faces = classe => Array.from({
      length: FACES_CRISTAL
    }, (_, i) => `<span class="${classe}" style="--i:${i}"></span>`).join("");
    const repetir = (classe, quantas) => Array.from({
      length: quantas
    }, (_, i) => `<i class="${classe}" style="--i:${i}"></i>`).join("");
  ...
  function cristalMeta() {
    ...
    ligarPonteiro(cena, (cx, cy) => {
      ...
      px = (cx - r.left) / r.width - .5;
      py = ((cy - r.top) / r.height - .5) * -1;
      agendar();
    }, () => {
      px = 0;
      py = 0;
  ...
  function mirarAlvo(progresso, nome, nota) {
    ...
    if (arco) {
      arco.style.strokeDashoffset = (VOLTA_ARCO * (1 - p / 100)).toFixed(2);
    }
    if (pct) {
      pct.textContent = nome ? `${Math.round(p)}%` : "—";
    }
    if (rotulo) {
      rotulo.textContent = nome || "sem metas ainda";
    }
    if (falta) {
      falta.textContent = nome ? nota || "" : "";
    }
  }
  const CAMADAS_EXTRUSAO = 26;
  function montarMarca(cena) {
    if (cena.dataset.montada) {
      return;
    }
    ...
    const extrusao = Array.from({
      length: CAMADAS_EXTRUSAO
    }, (_, i) => {
      const t = i / (CAMADAS_EXTRUSAO - 1);
      const z = (i + 1) * 1.5;
      const forca = (1 - t) ** 1.25;
      const matiz = t < .45 ? "var(--marca-metal-claro)" : t < .8 ? "var(--marca-metal)" : "var(--marca-metal-fundo)";
      return `<span class="marca3d__borda marca3d__extrusao"\n        style="--z:${z};--forca:${forca.toFixed(3)};--matiz:${matiz}"></span>`;
    }).join("");
    const faiscas = Array.from({
      length: 6
    }, (_, i) => `<span class="marca3d__faisca marca3d__faisca--${i + 1}"></span>`).join("");
  ...
  function marcaReality() {
    ...
    cenas.forEach(cena => {
      ...
      area.addEventListener("pointermove", e => {
        ...
        my = ((e.clientY - r.top) / r.height - .5) * -1;
        agendar();
      }, {
        passive: true
      });
