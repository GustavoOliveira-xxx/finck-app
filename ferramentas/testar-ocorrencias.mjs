...
eval(readFileSync(resolve(raiz, "js/fechamento.js"), "utf8"));
...
const perto = (t, obtido, esperado, tol = .01) => {
...
conferir("dia 31 em setembro cai em 30", ger.find(g => g.recurring_id === "net" && g.cycle === "2026-09").due_date, "2026-09-30");

conferir("dia 31 em agosto fica 31", ger.find(g => g.recurring_id === "net" && g.cycle === "2026-08").due_date, "2026-08-31");

conferir("nasce previsto", [ ...new Set(ger.map(g => g.status)) ], [ "previsto" ]);

conferir("valor previsto congelado", ger.find(g => g.recurring_id === "alu" && g.cycle === "2026-08").planned_amount, 1200);
...
const g1 = O.gerar(RECS, {
  referencia: HOJE
});

const g2 = O.gerar(RECS, {
  referencia: HOJE
});

const chave = g => `${g.recurring_id}|${g.cycle}`;

conferir("mesmas chaves nas duas passadas", g1.map(chave).sort().join(",") === g2.map(chave).sort().join(","), true);
...
conferir("agosto fecha depois do dia 31", O.cicloPronto(ocs, "2026-08", new Date(2026, 8, 1, 10, 0, 0)), true);
...
conferir("valor igual confirma", O.estadoAposValor(oc, 2e3), "confirmado");
...
conferir("saída leva categoria", O.movimentacaoDe({
  ...oc,
  type: "saida",
  category: "Moradia"
}, 100).category, "Moradia");

console.log("Referência de renda livre e economia");

const soPct = {
  free_income_percent: 30
};

const soValor = {
  free_income_amount: 600
};

const ambosPct = {
  free_income_mode: "percentual",
  free_income_percent: 30,
  free_income_amount: 600
};

const ambosValor = {
  free_income_mode: "valor",
  free_income_percent: 30,
  free_income_amount: 600
};

perto("só percentual", F.referencia(soPct, "free_income", 3e3).valor, 900);

perto("só valor fixo", F.referencia(soValor, "free_income", 3e3).valor, 600);

perto("ambos, modo percentual", F.referencia(ambosPct, "free_income", 3e3).valor, 900);

perto("ambos, modo valor", F.referencia(ambosValor, "free_income", 3e3).valor, 600);

conferir("nunca soma os dois", F.referencia(ambosPct, "free_income", 3e3).valor === 1500, false);

perto("sem configuração é zero", F.referencia({}, "free_income", 3e3).valor, 0);

console.log("Referência proporcional à receita real");

perto("percentual acompanha a receita real", F.proporcional(soPct, "free_income", 3e3, 2700), 810);

perto("valor fixo encolhe na proporção", F.proporcional(soValor, "free_income", 3e3, 2700), 540);
...
perto("receita esperada", r.receitaEsperada, 3e3);
