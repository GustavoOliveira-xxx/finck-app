...
eval(readFileSync(resolve(raiz, "js/ocorrencias.js"), "utf8"));

eval(readFileSync(resolve(raiz, "js/programacao.js"), "utf8"));
...
conferir("dia 31 em mês de 30 dias cai no último", P.proximaData(31, new Date(2026, 8, 15, 10, 0, 0)).toISOString().slice(0, 10), "2026-09-30");

conferir("dia 30 em fevereiro cai em 28", P.proximaData(30, new Date(2027, 1, 5, 10, 0, 0)).toISOString().slice(0, 10), "2027-02-28");

conferir("virada de ano", P.proximaData(3, new Date(2026, 11, 20, 10, 0, 0)).toISOString().slice(0, 10), "2027-01-03");
...
conferir("um marco por data", pan.marcos.map(m => [ m.iso, m.acumulado ]), [ [ "2026-08-15", 50 ], [ "2026-08-18", 150 ], [ "2026-08-20", 850 ] ]);

const doisNoMesmoDia = P.panorama({
  recorrentes: [ {
    id: "x",
    description: "A",
    type: "saida",
...
conferir("mesmo dia soma num marco só", doisNoMesmoDia.marcos.map(m => [ m.iso, m.acumulado ]), [ [ "2026-08-20", 400 ], [ "2026-09-20", 800 ] ]);
...
conferir("conta mensal aparece duas vezes em 60 dias", doisMeses.compromissos.map(c => c.iso), [ "2026-09-10", "2026-10-10" ]);

conferir("total soma as duas", doisMeses.comprometidoTotal, 198);

console.log("Projeção não inventa datas passadas");

conferir("dia 10 já passou e não entra sem ocorrência", P.panorama({
  recorrentes: mensal
}, 1e3, {
  referencia: HOJE,
  dias: 30
...
conferir("prevista ainda não vencida também conta", comOcs([ {
  ...base("previsto"),
  due_date: "2026-08-25"
} ]).comprometidoTotal, 99);

conferir("confirmada sai da lista, o saldo já caiu", comOcs([ base("confirmado", {
  actual_amount: 99
}) ]).comprometidoTotal, 0);

conferir("ajustada também sai", comOcs([ base("ajustado", {
  actual_amount: 120
}) ]).comprometidoTotal, 0);

conferir("não paga sai, a decisão é respeitada", comOcs([ base("nao_pago") ]).comprometidoTotal, 0);

conferir("não realizada sai", comOcs([ base("nao_realizado") ]).comprometidoTotal, 0);

conferir("entrada prevista nunca vira compromisso", comOcs([ {
  ...base("pendente"),
  type: "entrada"
} ]).comprometidoTotal, 0);
...
conferir("com a regra junto, o ciclo coberto não volta pela projeção", semDobra.compromissos.map(c => c.iso), [ "2026-09-10" ]);
...
conferir("ocorrência e regra do mesmo ciclo não somam duas vezes", dobraProibida.comprometidoTotal, 198);

conferir("a data que vale é a da ocorrência, não a da regra", dobraProibida.compromissos.map(c => c.iso), [ "2026-08-20", "2026-09-10" ]);
...
conferir("além do ciclo coberto a regra volta a projetar", horizonte.compromissos.map(c => c.iso), [ "2026-08-10", "2026-09-10", "2026-10-10" ]);

console.log("Vencidas somam e aparecem no total");

const duasVencidas = P.panorama({
  ocorrencias: [ {
    ...base("pendente"),
    id: "v1",
    due_date: "2026-08-05"
...
conferir("fonte ausente não quebra", P.panorama(undefined, 500, {
  referencia: HOJE
}).compromissos, []);

console.log("Saldo negativo após compromissos");

const apertado = P.panorama({
  recorrentes: recs
}, 400, {
  referencia: HOJE,
  dias: 30
